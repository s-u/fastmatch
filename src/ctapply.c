...
SEXP ctapply(SEXP args) {
    SEXP rho, vec, by, fun, mfun, cdi = 0, cdv = 0, tmp, acc, acctail;
    int i = 0, n, cdlen;
	...
	PROTECT(eres);
	if (!acc)
	    acctail = acc = SET_VECTOR_ELT(tmp, 2, CONS(eres, R_NilValue));
	else
	    acctail = SETCDR(acctail, CONS(eres, R_NilValue));
	{
	    char cbuf[64];
	    const char *name = "";
